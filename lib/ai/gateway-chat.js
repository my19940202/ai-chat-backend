...
export async function streamChatCompletion(messages, opts = {}) {
  const model = opts.model || getEnv('DEFAULT_MODEL', 'openai/gpt-5.4-mini');
  ...
  // OpenAI 路由传 max_tokens 会报错，由 provider 使用默认上限
  const params = {
    messages,
    stream: true,
    temperature,
...
export async function chatCompletion(messages, opts = {}) {
  ...
  if (!isOpenAIModel(model)) {
    runParams.max_tokens = opts.maxTokens ?? 256;
  }

  const res = await ai.run(model, runParams, { gateway: { id: gatewayId } });
